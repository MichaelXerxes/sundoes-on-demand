...
export function useOrderDetails() {
  ...
  if (!context) {
    throw new Error(
      "useOrderDetails must be called from within an OrderDetailsProvicder"
    );
  }
...
export function OrderDetailsProvider(props) {
  const [optionCounts, setOptionCounts] = useState({
    ...
    toppings: {}, //exa,ple {"Gummi Bears":1}
  });
  function updateItemCount(itemName, newItemCount, optionType) {
    ...
    //set , update the state with the updated copy
  ...
  function calculateTotal(optionType) {
    ...
    //total the values in teh array of counts for the number of items
  ...
  const totals = {
    ...
    toppings: calculateTotal("toopings"),
  };
